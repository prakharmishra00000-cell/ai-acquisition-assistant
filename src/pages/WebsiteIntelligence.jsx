    ...
    <div className="flex-col gap-8">
      ...
      <div className="glass-panel p-8" style={{ background: 'linear-gradient(135deg, rgba(18,21,38,0.8), rgba(28,33,54,0.9))' }}>
        <div className="flex justify-between items-center">
          <div>
            <div className="text-secondary font-medium mb-1">AI Estimated Valuation Range</div>
          ...
          <div className="text-right">
            ...
            <div style={{ fontSize: '2rem', fontWeight: 600, color: 'var(--accent-green)' }}>89%</div>
          </div>
        </div>
        ...
        <div className="mt-6 p-4" style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '8px', borderLeft: '4px solid var(--accent-blue)' }}>
          ...
          <p className="text-sm text-secondary leading-relaxed">
            "An emerging AI SaaS platform with a modern Next.js/React frontend and a robust Node.js backend. It leverages OpenAI's models for core functionality. With strong mobile responsiveness and scalable architecture, it represents a highly attractive acquisition target for private equity looking to enter the AI design space."
          </p>
        </div>
      </div>

      {/* Tech Stack Analysis */}
      <h2 style={{ fontSize: '1.5rem', marginTop: '1rem' }}>Tech Stack Detection</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
        <IntelligenceCard title="Frontend" icon={Smartphone} items={['React', 'Next.js', 'Tailwind CSS', 'Framer Motion']} />
        <IntelligenceCard title="Backend & DB" icon={Server} items={['Node.js', 'Express', 'PostgreSQL', 'Redis']} />
        <IntelligenceCard title="AI Models" icon={Cpu} items={['OpenAI GPT-4', 'DALL-E 3', 'Pinecone Vector DB']} />
        <IntelligenceCard title="Security & Auth" icon={Shield} items={['Clerk Auth', 'JWT', 'HTTPS Enforced']} />
        <IntelligenceCard title="Performance" icon={Zap} items={['98/100 Lighthouse', 'Edge Caching (Vercel)']} />
        <IntelligenceCard title="SEO & Analytics" icon={LineChart} items={['Semantic HTML', 'PostHog', 'Sitemap.xml']} />
      </div>
    </div>
  );
};
